...
'membuat query'
 ...
 tgldb2=thn2+"-"+bln2+"-"+tgl2+" 23:59:59"
  if (userid="ARDI") or (userid="M") or (userid="R") then userid = "CA001" end if
						...
						querytbl =  "SELECT    kd_permintaan, tgl_permintaan, nopol, nomaster, nama, bagian, seksi, kuantum, km_awal, km_akhir, status, tgl_settlement, nota_tgl, nota_nomor, nota_quantum, updated_by, kuantum_acc, tujuan, nota_uang, nota_ref, bahanbakar, hargabbm, asalspbu, acc_skw, acc_skk, isnull(acc_ca,0) as acc_ca,tglacc_ca FROM   tblPermintaanBBM WHERE    (bagian = 'Tanaman') AND CONVERT(date, tgl_permintaan, 120) >= '"&tgldb&"' AND CONVERT(date, tgl_permintaan, 120) <='"&tgldb2&"' AND nopol not like '%batal%' ORDER BY acc_ca, convert(date, tgl_permintaan, 120), nama"
