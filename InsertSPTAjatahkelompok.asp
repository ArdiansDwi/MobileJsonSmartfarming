...
<% 
 ...
 On Error Resume Next 
						...
						register = Request.QueryString("kdspa")
						UserId = Request.QueryString("userid") 
						Pos =  Request.QueryString("pos") 

						qtgl = "select replace(convert(varchar, getdate(),112),'-','') + replace(convert(varchar, getdate(),108),':','') as tglkodespa"
						set rd = server.CreateObject("ADODB.RECORDSET")
						rd.Open qtgl, conn,3,1
						while not rd.eof
							tglkodespa = rd.fields ("tglkodespa")
							kdinsertspta= tglkodespa+"#"+UserId
							rd.movenext 
						wend
						...
						'insert kuota SPTA tblKuotaNoInduk
						qinsert = "insert into tblkuotanoinduk (kdspa,NoEntry,tgl_entry, kdptn,Nama,kdafdeling,afdeling,jml,status,bap,tmp,jmlcetak,Cetak,userlogin,TglTerakhirCetak,kdpos,pos,tglberlaku) (select top 1 '" & kdinsertspta & "','" & kdinsertspta & "',getdate() as tgl_entry,KD_PTN,NM_PTN,b.kd,a.afdl,1,1,0,1,0,'Mobile'+'" & UserId & "','" & UserId & "',getdate(),(select top 1 kd from tblpostebu where upper(pos) = '" & pos & "'),'" & pos & "',case when datepart(hour,getdate())<6 then convert(date,getdate()-1,120) else convert(date,getdate(),120) end as tglberlaku  from m_ptnspta a inner join tblafdl b on a.afdl = b.afdl where kd_ptn='" & register & "' )"
	...
	'insert ke Tabel tblspta_mobile
	 qinsert = "insert into tblspta_mobile(tglspta, Register, Nama, Afdeling, NoSPTA, KodeSticker, NoKendaraan, Pos, x, y, Lokasi, Foto1, Foto2, Keterangan, kdspa,userid,Petugas,KodeInsert) values ('" & Request.QueryString("tglspta") & "','" & Register  & "','" & Nama & "','" & Afdeling & "','" & Request.QueryString("nospta") & "','" & Request.QueryString("kodesticker") & "',UPPER('" & Request.QueryString("nokendaraan") & "'),'" & Request.QueryString("pos") & "','" & Request.QueryString("x") & "','" & Request.QueryString("y") & "','" & Request.QueryString("lokasi") & "','" & Request.QueryString("foto1") & "','" & Request.QueryString("foto2") & "','" & Request.QueryString("keterangan") & "','" & kdinsertspta & "','" & Request.QueryString("userid") & "','" & Request.QueryString("Petugas") & "','" & KodeInsert & "')"
